...
def give_prot(search_chain, dico):
    """
    ...
    string
    """
    for protein in dico.keys():
        for chain in dico[protein]:
...
def create_df(lst, dico):
    ...
    for c in lst:
        for i in c[1]:
            data['protein1'].append(give_prot(i[0], dico))
            data['chain1'].append(i[0])
            data['res1'].append(i[1])
            data['distance'].append(i[2])
            data['protein2'].append(give_prot(i[3], dico))
        ...
        if c[2] != []:
            for j in c[2]:
                data['protein1'].append(give_prot(j[0], dico))
                data['chain1'].append(j[0])
                data['res1'].append(j[1])
                data['distance'].append(j[2])
                data['protein2'].append(give_prot(j[3], dico))
